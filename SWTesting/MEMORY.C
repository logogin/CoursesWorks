...
void main (void)
{
 ...
 if((fptr=fopen(pFilename,"r"))==NULL){
  printf("\aCan't open file!"); exit(1);
 }
 check(fptr,str);
 fclose(fptr);
}
...
void check(FILE *fptr, char **str)
{
  ...
  do{
     ...
     switch(count){
    case  0:{ SInt+=integer  (row);  break;}
    case  1:{ SChar+=character(row);  break;}
    case  2:{ SFloat+=floater  (row);  break;}
    case  3:{ SDouble+=doubler  (row);  break;}
    case  4:{ SLong+=longer   (row);  break;}
    default:{ printf("\aERROR!");    return ;}
     }
     }
  }
  }while(!feof(fptr));
